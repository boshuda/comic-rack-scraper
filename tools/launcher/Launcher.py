'''
This module's purpose is to launch the ComicRackScraper plugin from within the 
...
ComicRackScraper distributable and will not be run by regular users.
'''
...
# 1. add our 'fake' ComicRack into ComicRackScraper, in the same spot where it 
#    would be if we were running inside the real ComicRack app.
import ComicRackScraper
ComicRackScraper.ComicRack = ComicRack

# 2. now go ahead and start the ComicRackScraper plugin.
if len(sys.argv) == 2:
      ...
      ComicRackScraper.cvs_scrape(books)  
else:
